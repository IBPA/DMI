...
concentrations_ind = sapply(metabolite_nodes, function(node){
  ...
  for (cur_concentration_field in concentration_fields){
    ...
    print(paste0("Found concentration : ", conc_value, " ", conc_units))
    return(T)
  }
...
write.csv(cbind(mcdb_summary, find_mapping_dmd_id),paste0(PROCESSED_DATA_PATH,"\\SuppFig1_2_SuppTable1_OmicsSetComparison\\SmallMolecules_Comparison_MCDB.csv"))
